...
double scaledReprojError(cv::vector<cv::Point2f> imagePoints, cv::vector<cv::Point2f> reprojImagePoints);
void ledFinder(std::vector<cv::Point2f> imagePoints, std::vector<cv::Point2f> &newImagePoints, int callNo = 1);
bool pairComparator(std::pair<double,int> a, std::pair<double,int> b) {return a.second > b.second;}
...
void ledFinder(std::vector<cv::Point2f> imagePoints, std::vector<cv::Point2f> &newImagePoints, int callNo) {
	...
	std::sort(point3Pairs.begin(), point3Pairs.end(), pairComparator);
	...
	for (int i=0; i<NO_LEDS; i++) {
		...
		newImagePoints[2] = imagePoints[point5];
